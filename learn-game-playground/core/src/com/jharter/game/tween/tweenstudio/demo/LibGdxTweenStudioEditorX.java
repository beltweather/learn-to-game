...
public class LibGdxTweenStudioEditorX extends Editor {
	...
	private final ImmediateModeRenderer imr = new ImmediateModeRenderer20(false, false, 0);
	...
	private void drawBoundingBox(Matrix4 projModelView, GL20 gl, Sprite sp, Color color) {
		...
		Matrix4 matrix = projModelView.cpy();
		matrix.translate(+sp.getX()+sp.getOriginX(), +sp.getY()+sp.getOriginY(), 0);
		matrix.rotate(sp.getRotation(), 0, 0, 1);
		matrix.translate(-sp.getX()-sp.getOriginX(), -sp.getY()-sp.getOriginY(), 0);
		
		Rectangle bb = getBoundingBox(sp);
		drawRect(matrix, bb.x, bb.y, bb.width, bb.height, color);
