...
public abstract class CursorSystem extends FirstSystem {
	...
	protected float getZoneCenterY(Entity cursor, ZoneComp z) {
		float centerY = 0;
	...
	protected float getCursorAngle(ZoneType zoneType) {
		switch(zoneType) {
			case FRIEND:
	...
	protected Vector3 getCursorPosition(ID cursorID) {
		return getCursorPosition(Comp.Entity.get(cursorID));
	}
	
	protected Vector3 getCursorPosition(Entity cursor) {
		return getCursorPosition(cursor, Comp.CursorComp.get(cursor).targetID);
	}
	
	protected Vector3 getCursorPosition(ID cursorID, ID targetID) {
		return getCursorPosition(Comp.Entity.get(cursorID), targetID);
	}
	
	protected Vector3 getCursorPosition(Entity cursor, ID targetID) {
		return getCursorPosition(cursor, targetID, Comp.ZoneComp.get(Comp.ZonePositionComp.get(targetID).zoneID));
	}
	
	protected Vector3 getCursorPosition(ID cursorID, ID targetID, ZoneComp z) {
		return getCursorPosition(Comp.Entity.get(cursorID), targetID, z);
	}
	
	protected Vector3 getCursorPosition(Entity cursor, ID targetID, ZoneComp z) {
		if(targetID == null) {
			return null;
		}
	...
	protected boolean isAll(ID cursorID) {
		return isAll(Comp.CursorComp.get(cursorID));
	}
	
	protected boolean isAll(Entity cursor) {
		return isAll(Comp.CursorComp.get(cursor));
	}
	
	protected boolean isAll(CursorComp c) {
		TurnAction t = getTurnAction(c);
		return t != null && t.all;
	}
	
	protected Array<ID> getCursorAllIDs(CursorComp c) {
	...
	protected boolean isTargetingCard(CursorComp c) {
	...
	protected Array<ID> getCursorSecondaryIDs(CursorComp c) {
